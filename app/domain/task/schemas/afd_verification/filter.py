...
from .task import AfdClusterModel, AfdVerificationModel
...
def show_lhs_rhs_only(raw_result: AfdVerificationModel, 
                      is_show_lhs_rhs_only: bool) -> AfdVerificationModel:
    ...
    for cluster in raw_result['clusters']:
        filtered_rows = [
            [row[i] for i in raw_result['lhs_rhs_indices']]
    ...
    raw_result['table_header'] = [raw_result['table_header'][i] for i in raw_result['lhs_rhs_indices']]
    return raw_result
...
class AfdVerificationFilter(BaseFilter):
    ...
    def filter(self, 
               ...
               filter_params: List[str]) -> AfdVerificationModel:
