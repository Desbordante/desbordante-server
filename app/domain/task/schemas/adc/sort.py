...
def sort_by_attributes(raw_result: List[AdcModel], is_reverse: bool) -> List[AdcModel]:
    raw_result.sort(key=json.dumps, reverse=is_reverse)
    return raw_result


def sort_by_len(raw_result: List[AdcModel], is_reverse: bool) -> List[AdcModel]:
    raw_result.sort(key=lambda x: len(x["cojuncts"]), reverse=is_reverse)
    return raw_result
