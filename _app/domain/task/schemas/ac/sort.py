...
def sort_by_attributes(raw_result: List[AcModel], is_reverse: bool) -> List[AcModel]:
    raw_result.sort(key=json.dumps, reverse=is_reverse)
    return raw_result


def sort_by_intervals(raw_result: List[AcModel], is_reverse: bool) -> List[AcModel]:
    raw_result.sort(key=lambda x: len(x["intervals"]), reverse=is_reverse)
    return raw_result


def sort_by_outliers(raw_result: List[AcModel], is_reverse: bool) -> List[AcModel]:
    raw_result.sort(key=lambda x: len(x["outliers"]), reverse=is_reverse)
    return raw_result
