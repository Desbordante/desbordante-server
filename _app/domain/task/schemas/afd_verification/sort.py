...
def sort_by_frequentness(
    raw_result: AfdVerificationModel, is_reverse: bool
) -> AfdVerificationModel:
    raw_result["clusters"].sort(
        key=lambda x: x["most_frequent_rhs_value_proportion"], reverse=is_reverse
    )
    return raw_result
...
def sort_by_size(
    ...
    raw_result["clusters"].sort(key=lambda x: len(x["rows"]), reverse=is_reverse)
    return raw_result
...
def sort_by_rhs(
    ...
    raw_result["clusters"].sort(
        key=lambda x: x["num_distinct_rhs_values"], reverse=is_reverse
    )
    return raw_result
