"""Integration tests for ProfilingDepCrud.get_many (FD primitive)."""
...
from src.schemas.task_schemas.primitives.fd.task_result import FdTaskResultOrderingField
from tests.integration.crud.profiling_dep_crud.fd.helpers import make_query_params

pytestmark = pytest.mark.asyncio


async def test_get_many_filter_search(
    fd_profiling_dep_crud: ProfilingDepCrud,
    many_fd_deps: list[ProfilingDepModel],
) -> None:
    task_id = many_fd_deps[0].task_id

    pagination, query_params = make_query_params(search="alpha")
    result = await fd_profiling_dep_crud.get_many(
...
@pytest.mark.parametrize(
    ...
    [
        (
            FdTaskResultOrderingField.RHS_COLUMN_NAME,
